...
def scrape_and_save():
    ...
    for url in urls:
        ...
        if response.status_code == 200:
            ...
            for username, upvote in zip(usernames, upvotes):
                ...
                # Fetch true score from user profile
                profile_url = f"https://pcmemes.net/@{username_text}"
                profile_response = requests.get(profile_url)
                if profile_response.status_code == 200:
                    profile_soup = BeautifulSoup(profile_response.content, 'html.parser')
                    true_score_element = profile_soup.select_one('#profile--info--truescore')
                    if true_score_element:
                        true_score_text = true_score_element.get_text().strip()
                        true_score_match = re.search(r'(\d+)', true_score_text)
                        if true_score_match:
                            true_score = int(true_score_match.group(1))
                            data_dict[username_text] = true_score
    ...
    data = [{'username': k, 'score': v, 'profile': f'https://pcmemes.net/@{k}', 'true_score': data_dict[k]} for k, v in data_dict.items()]
